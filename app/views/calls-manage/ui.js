...
const Control = p => (
  <View style={st.control}>
    <View style={st.controlCall}>
      {!p.answered &&
        p.incoming && <Text style={st.callStatus}>Ringing by</Text>}
      {!p.answered &&
        !p.incoming && <Text style={st.callStatus}>Calling to</Text>}
      {p.answered && p.holding && <Text style={st.callStatus}>Holding on</Text>}
      {p.answered &&
        !p.holding && <Text style={st.callStatus}>Talking to</Text>}
...
const RunningItem = p => (
  <Button style={st.call} onPress={p.select} disabled={p.selected}>
    ...
    {p.answered &&
      p.holding && <Text style={st.callIconHolding}>icon_phone_pause</Text>}
    {p.answered &&
      !p.holding && <Text style={st.callIconTalking}>icon_phone_call</Text>}
